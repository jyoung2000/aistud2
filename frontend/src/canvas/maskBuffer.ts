...
/** Separable box min-filter (window 2r+1) — erosion, coverage-preserving. */
function boxMin(src: Uint8Array, w: number, h: number, r: number): Uint8Array {
...
export class MaskBuffer {
  ...
  /** Erode (shrink) by r px — box-min on the coverage channel so soft edges survive. */
  shrink(r: number): void {
    if (r <= 0) return;
    this.data = boxMin(this.data, this.width, this.height, r);
  }
